...
describe('SPEC-023-1-05 deploy lifecycle: local', () => {
  ...
  it('build → deploy → healthCheck → rollback completes end-to-end', async () => {
    ...
    const recordPath = join(repoPath, '.autonomous-dev', 'deployments', `${record.deployId}.json`);
    ...
    const closeCall = m
      .calls()
      .find((c) => c.cmd === 'gh' && c.args[0] === 'pr' && c.args[1] === 'close');
...
describe('SPEC-023-1-05 deploy lifecycle: static', () => {
  ...
  it('build → deploy → second deploy → rollback restores first artifact', async () => {
    const m = makeRunToolMock();
    ...
    const ctx2: BuildContext = {
      ...ctx1,
      requestId: 'req-int-static-2',
      params: { build_command: 'echo v2', build_dir: 'dist2' },
    };
