...
): jest.Mocked<Pick<DocumentStorage, 'listVersions' | 'writeVersion'>> & DocumentStorage {
  const mock = {
...
describe('createVersion', () => {
  ...
  it('calls calculateNextVersion with correct args', async () => {
    ...
    expect(storage.writeVersion).toHaveBeenCalledWith(expect.objectContaining({ version: '1.1' }));
  });
  ...
  it('with REVIEW_REVISION increments from latest version', async () => {
    ...
    expect(storage.writeVersion).toHaveBeenCalledWith(expect.objectContaining({ version: '1.3' }));
  });
});
