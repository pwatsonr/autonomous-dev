...
// ---------------------------------------------------------------------------
function validFrontmatter(overrides?: Partial<DocumentFrontmatter>): Partial<DocumentFrontmatter> {
...
function test_returns_error_when_pipeline_id_does_not_match_regex(): void {
  ...
  const pipeError = result.errors.find(
    (e) => e.field === 'pipeline_id' && e.code === 'INVALID_FORMAT',
  );
...
function test_returns_error_when_version_does_not_match_regex(): void {
  ...
  const versionError = result.errors.find(
    (e) => e.field === 'version' && e.code === 'INVALID_FORMAT',
  );
...
function test_returns_error_when_depth_does_not_match_type(): void {
  ...
  const depthError = result.errors.find(
    (e) => e.field === 'depth' && e.code === 'DEPTH_TYPE_MISMATCH',
  );
...
function test_returns_error_when_updated_at_before_created_at(): void {
  ...
  const tsError = result.errors.find(
    (e) => e.field === 'updated_at' && e.code === 'TIMESTAMP_ORDER',
  );
...
function test_returns_error_when_depends_on_length_mismatch(): void {
  ...
  const result = validateFrontmatter(fm);
  const lenError = result.errors.find(
    (e) => e.field === 'dependency_type' && e.code === 'ARRAY_LENGTH_MISMATCH',
  );
...
function test_returns_error_when_sibling_index_gte_count(): void {
  ...
  const result = validateFrontmatter(fm);
  const sibError = result.errors.find(
    (e) => e.field === 'sibling_index' && e.code === 'SIBLING_INDEX_OUT_OF_RANGE',
  );
...
function test_returns_error_when_parent_id_null_and_depth_gt_0(): void {
  ...
  const parentError = result.errors.find(
    (e) => e.field === 'parent_id' && e.code === 'PARENT_ID_REQUIRED',
  );
  assert(
    parentError !== undefined,
    `null parent_id at depth > 0 should produce PARENT_ID_REQUIRED error`,
  );
...
function test_returns_error_when_parent_id_nonnull_and_depth_0(): void {
  ...
  const result = validateFrontmatter(fm);
  const parentError = result.errors.find(
    (e) => e.field === 'parent_id' && e.code === 'PARENT_ID_DEPTH_MISMATCH',
  );
  assert(
    parentError !== undefined,
    `non-null parent_id at depth 0 should produce PARENT_ID_DEPTH_MISMATCH error`,
  );
...
function test_returns_warning_for_unknown_fields(): void {
  ...
  const unknownWarning = result.warnings.find(
    (w) => w.field === 'custom_field' && w.code === 'UNKNOWN_FIELD',
  );
...
function test_returns_error_when_traces_from_empty_for_depth_gt_0(): void {
  ...
  const result = validateFrontmatter(fm);
  const tracesError = result.errors.find(
    (e) => e.field === 'traces_from' && e.code === 'TRACES_FROM_REQUIRED',
  );
  assert(
    tracesError !== undefined,
    `Empty traces_from at depth > 0 should produce TRACES_FROM_REQUIRED error`,
  );
