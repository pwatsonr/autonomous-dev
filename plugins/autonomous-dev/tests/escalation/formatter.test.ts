...
describe('EscalationIdGenerator', () => {
  ...
  test('counter resets to 001 on date change', () => {
    ...
    const gen = new EscalationIdGenerator(statePath, () => currentDate);
  ...
  test('counter above 999 is not truncated', () => {
    ...
    fs.writeFileSync(statePath, JSON.stringify({ date: FIXED_DATE, counter: 999 }));
...
describe('EscalationFormatter', () => {
  ...
  test('standard mode omits technical_details', () => {
    const formatter = createFormatter('standard');
    const result = formatter.format(makeInput({ technicalDetails: 'Extended stack trace...' }));
  ...
  test('secret values are redacted from summary', () => {
    const formatter = createFormatter('standard');
    const result = formatter.format(makeInput({ failureReason: 'Failed: api_key=sk-12345abc' }));
  ...
  test('secret values are redacted from failure_reason', () => {
    ...
    const result = formatter.format(
      makeInput({
        failureReason: 'Connection string: connection_string=postgres://user:pass@host',
      }),
    );
  ...
  test('technical_details with file paths are NOT redacted', () => {
    const formatter = createFormatter('verbose');
    const result = formatter.format(makeInput({ technicalDetails: 'Error at src/foo.ts:42' }));
  ...
  test('previous_escalation_id appears when provided', () => {
    const formatter = createFormatter('standard');
    const result = formatter.format(makeInput({ previousEscalationId: 'esc-20260407-005' }));
...
describe('redactSecrets', () => {
  test('redacts api_key=value', () => {
    expect(redactSecrets('Failed: api_key=sk-12345abc')).toBe('Failed: [REDACTED]');
  });

  test('redacts token=value', () => {
    expect(redactSecrets('Use token=abc123 for auth')).toBe('Use [REDACTED] for auth');
  });
  ...
  test('redacts connection_string=value', () => {
    expect(redactSecrets('connection_string=postgres://user:pass@host')).toBe('[REDACTED]');
  });
  ...
  test('does not redact normal text', () => {
    expect(redactSecrets('Error at src/foo.ts:42')).toBe('Error at src/foo.ts:42');
  });
...
describe('sanitizePath', () => {
  test('strips workspace root prefix', () => {
    expect(sanitizePath('/Users/dev/workspace/repo/src/main.ts', '/Users/dev/workspace/repo')).toBe(
      'src/main.ts',
    );
  });

  test('returns path as-is when workspace root does not match', () => {
    expect(sanitizePath('/other/location/file.ts', '/Users/dev/workspace/repo')).toBe(
      '/other/location/file.ts',
    );
  });

  test('strips leading slash after workspace root removal', () => {
    expect(sanitizePath('/Users/dev/workspace/repo/file.ts', '/Users/dev/workspace/repo')).toBe(
      'file.ts',
    );
  });
});
