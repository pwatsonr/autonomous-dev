...
function test_vault_detector(): void {
  ...
  assert(
    vaultDetector.detect('a/b', [{ topic: 'dependencies', content: 'express only' }]).length === 0,
    'no false positive',
  );
...
function test_test_and_domain_detectors(): void {
  ...
  assert(
    dg.length === 1 && dg[0].suggestedName === 'domain-context',
    'rich STRUCTURED overview → domain-context',
  );
  // a short overview is below threshold → no opportunity
  assert(
    domainGlossaryDetector.detect('a/b', [{ topic: 'overview', content: '# Tiny' }]).length === 0,
    'short overview skipped',
  );
  // a long but UNSTRUCTURED prose readme (no headings/glossary) → no opportunity (H3 noise cut)
  assert(
    domainGlossaryDetector.detect('a/b', [
      { topic: 'overview', content: 'flat prose. '.repeat(80) },
    ]).length === 0,
...
function test_detect_orchestrator_reads_memory(): void {
  ...
  writeMemoryDoc(
    'repo:acme/api',
    'test-conventions',
    '# Test conventions — acme/api\n\nDetected: jest.config.cjs',
    io,
  );
  const res = detectOpportunities('acme/api', io);
  const names = res.opportunities.map((o) => o.suggestedName).sort();
  assert(
    names.join(',') === 'run-tests,vault-access',
    `vault + test opportunities, got ${names.join(',')}`,
  );
...
function test_detector_failure_isolated(): void {
  ...
  assert(
    res.opportunities.some((o) => o.suggestedName === 'vault-access'),
