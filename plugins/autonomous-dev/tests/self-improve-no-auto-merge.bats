...
@test "T011-06: checkAutoMergeAllowed rejects source=self-improve" {
  ...
  [ -f "${MERGE_GATE_TS}" ] || {
    ...
    fail "merge_gate.ts missing"
  }

  # Use npx ts-node to run a tiny inline test
  if command -v npx &>/dev/null; then
    local result
    result=$(cd "${PLUGIN_DIR}" && npx ts-node --compiler-options '{"module":"commonjs"}' \
      --eval "
const mg = require('./intake/triggers/self_improve/merge_gate');
const r = mg.checkAutoMergeAllowed({ source: 'self-improve' }, []);
...
" 2>&1) || true
    ...
    echo "${result}" | grep -q "ok: allow=false" || {
      echo "Test result: ${result}" >&2
      fail "merge_gate did not return allow=false for self-improve source"
    }
    return 0
  fi

  # Fallback: verify the TypeScript source directly with grep
  # (sufficient when ts-node is unavailable in CI)
  grep -q "never auto-merges" "${MERGE_GATE_TS}" || \
    fail "merge_gate.ts does not contain 'never auto-merges' (FR-MERGE-01)"
  grep -q "self-improve" "${MERGE_GATE_TS}" || \
    fail "merge_gate.ts does not reference 'self-improve'"
...
@test "T011-06b: isSelfImproveRequest returns true for state.source=self-improve" {
  local MERGE_GATE_TS="${PLUGIN_DIR}/intake/triggers/self_improve/merge_gate.ts"
  [ -f "${MERGE_GATE_TS}" ] || fail "merge_gate.ts missing"

  # Verify the source-based detection is implemented
  grep -q "source.*self-improve\|self-improve.*source" "${MERGE_GATE_TS}" || \
    fail "merge_gate.ts does not implement source-based self-improve detection"
}
...
@test "T011-07: no bin/lib/auto-merge helpers found (preventative guard)" {
  ...
  while IFS= read -r f; do
    grep -q "checkAutoMergeAllowed\|merge_gate\|self-improve" "${f}" || {
      echo "File ${f} invokes auto-merge without a self-improve gate" >&2
      fail "auto-merge helper '${f}' missing checkAutoMergeAllowed gate (FR-MERGE-01)"
    }
...
@test "T011-07b: supervisor-loop.sh auto-merge block documents the gate pattern" {
  ...
  # the gate lives.
  grep -q "trust\|merge.*gate\|merge_gate" "${supervisor}" || \
    fail "supervisor-loop.sh does not reference trust-level merge gate"
}
...
@test "T011-08: merge_gate.ts is state-based not label-based (ADR-006)" {
  ...
  grep -q "state\.source\|source === 'self-improve'" "${MERGE_GATE_TS}" || \
    fail "merge_gate.ts does not read state.source (ADR-006 violation)"
}
