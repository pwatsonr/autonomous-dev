...
Use the existing test runner (Jest, per the autonomous-dev convention). All test files end in `.test.ts`. Each test file:
...
- [ ] `jest --coverage` reports ≥95% line + branch coverage on `src/hooks/trust-validator.ts` and `src/hooks/signature-verifier.ts`.
...
- **Coverage tool**: enforce the 95% threshold via `jest --coverage --reporter=text --threshold 95` in the CI test script. Below-threshold runs fail the build.
...
- **Test run ordering**: each test file is independent; no `beforeAll` shared state. This makes `jest -t <single test>` work for any individual case.
