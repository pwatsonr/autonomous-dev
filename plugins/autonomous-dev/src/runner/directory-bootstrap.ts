 ...
 */
export async function bootstrapDirectories(rootDir: string, now: Date = new Date()): Promise<void> {
  ...
  const yearMonthPath = path.join(rootDir, '.autonomous-dev/observations', year, month);
