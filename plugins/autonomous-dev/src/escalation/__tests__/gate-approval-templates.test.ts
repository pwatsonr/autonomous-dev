...
describe('Gate Approval Templates - PRD Approval', () => {
  ...
  it('includes an approve option', () => {
    ...
    expect(approveOption!.description).toBe('Approve the PRD as-is and proceed to implementation');
  });
  ...
  it('includes a reject option', () => {
    ...
    expect(rejectOption!.description).toBe('Reject the PRD; pipeline will be cancelled');
  });

  it('includes an approve_with_conditions option', () => {
    const options = getGateApprovalTemplate('prd_approval');
    const condOption = options.find((opt) => opt.action === 'approve_with_conditions');
...
describe('Gate Approval Templates - Code Review', () => {
  ...
  it('has correct labels', () => {
    const options = getGateApprovalTemplate('code_review');
    expect(options.find((o) => o.action === 'approve')!.label).toBe('Approve code');
    expect(options.find((o) => o.action === 'retry_with_changes')!.label).toBe('Request changes');
    expect(options.find((o) => o.action === 'reject')!.label).toBe('Reject code');
  });
});
...
describe('Gate Approval Templates - Deployment Approval', () => {
  ...
  it('has correct labels', () => {
    const options = getGateApprovalTemplate('deployment_approval');
    expect(options.find((o) => o.action === 'approve')!.label).toBe('Approve deployment');
    expect(options.find((o) => o.action === 'reject')!.label).toBe('Reject deployment');
    expect(options.find((o) => o.action === 'cancel')!.label).toBe('Defer deployment');
  });
});
...
describe('Gate Approval Templates - Security Review', () => {
  ...
  it('has correct labels', () => {
    const options = getGateApprovalTemplate('security_review');
    expect(options.find((o) => o.action === 'approve')!.label).toBe('Approve (no findings)');
    expect(options.find((o) => o.action === 'retry_with_changes')!.label).toBe(
      'Remediate and retry',
    );
    expect(options.find((o) => o.action === 'reject')!.label).toBe('Reject (critical findings)');
  });
});
