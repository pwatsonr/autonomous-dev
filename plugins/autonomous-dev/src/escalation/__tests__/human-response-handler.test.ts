...
function makeEscalationStore(escalation: StoredEscalation | null): EscalationStore {
  return {
...
describe('HumanResponseHandler', () => {
  ...
  test("handleResponse('Use v2 API', escId, 'user') returns retry_with_changes", () => {
    ...
    const result = handler.handleResponse('Use v2 API', ESCALATION_ID, RESPONDER);
  ...
  test("handleResponse('delegate:default-user', escId, 'user') returns delegate action", () => {
    ...
    const result = handler.handleResponse('delegate:default-user', ESCALATION_ID, RESPONDER);
  ...
  test('handleResponse returns RESUME_FAILED when pipeline executor throws; escalation remains active', () => {
    const { handler } = createHandler(makePendingEscalation(), false, {
  ...
  test("handleResponse('opt-4', ...) triggers human_override audit event via resumption", () => {
    ...
    const result = handler.handleResponse('opt-4', ESCALATION_ID, RESPONDER);
  ...
  test('option with retry action injects guidance and re-executes', () => {
    ...
    expect(executor.injectGuidance).toHaveBeenCalledWith(REQUEST_ID, 'Retry with guidance');
  ...
  test('option with cancel action terminates the request', () => {
    ...
    expect(executor.terminateRequest).toHaveBeenCalledWith(REQUEST_ID, 'Cancelled by human');
  });
