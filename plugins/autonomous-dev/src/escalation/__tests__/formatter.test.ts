...
describe('EscalationFormatter', () => {
  function createFormatter(verbosity: 'terse' | 'standard' | 'verbose' = 'standard') {
    const statePath = path.join(tmpDir, 'counter.json');
  ...
  test('artifact paths are sanitized to workspace-relative', () => {
    const statePath = path.join(tmpDir, 'counter.json');
    const idGen = new EscalationIdGenerator(statePath, () => FIXED_DATE);
    const formatter = new EscalationFormatter(idGen, 'standard', '/Users/dev/workspace/repo');
...
describe('sanitizePath', () => {
  test('strips workspace root prefix', () => {
    expect(sanitizePath('/Users/dev/repo/src/file.ts', '/Users/dev/repo')).toBe('src/file.ts');
  });

  test('returns path unchanged when not under workspace root', () => {
    expect(sanitizePath('/other/path/file.ts', '/Users/dev/repo')).toBe('/other/path/file.ts');
  });
});
