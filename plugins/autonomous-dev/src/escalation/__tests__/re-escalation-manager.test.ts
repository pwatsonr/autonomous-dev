...
function createMockEscalationEngine() {
  ...
  return {
    raise: jest.fn(
      (failureContext: FailureContext, requestContext: RequestContext): EscalationResult => {
        callCount++;
...
describe('ReEscalationManager', () => {
  ...
  it('includes failure context in the re-escalation', () => {
    ...
    expect(msg.failure_reason).toBe('Compilation failed: missing dependency after guidance');
  });
  ...
  it('includes a cancel option in loop-detected escalation', () => {
    ...
    manager2.handlePostGuidanceFailure(
      'esc-002',
      createFailureContext(),
      createRequestContext(),
      'G1',
    ...
    const msg = manager2.handlePostGuidanceFailure(
      'esc-002',
      createFailureContext(),
      createRequestContext(),
      'G3',
