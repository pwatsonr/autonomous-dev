...
function makeConfig(
  knownTargetIds: string[] = ['default-user', 'security-lead', 'tech-lead'],
): EscalationConfig {
  return {
    routing: {
      mode: 'default',
...
describe('ResponseValidator', () => {
  ...
  test('passes validation for non-empty freetext', () => {
    ...
    const result = validator.validate(
      makeFreetextResponse('Please retry with a smaller batch size'),
    );
  ...
  test('check order: ESCALATION_NOT_FOUND takes priority over REQUEST_CANCELLED', () => {
    ...
    const validator = new ResponseValidator(makeStore(null), makeConfig(), makeKillSwitch(true));
    const result = validator.validate(makeOptionResponse('opt-1'));
  ...
  test('default target is included in known targets for delegation', () => {
    const validator = makeValidator(makePendingEscalation(), false, ['default-user']);
