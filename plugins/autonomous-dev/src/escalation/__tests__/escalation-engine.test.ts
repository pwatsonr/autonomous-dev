  ...
  return {
    format: jest.fn((input: FormatterInput): EscalationMessage => {
      ...
      return {
        ...
        summary: `[${input.escalationType}] ${input.pipelinePhase}: ${input.failureReason}`.slice(
          0,
          200,
        ),
...
describe('EscalationEngine', () => {
  ...
  test('infrastructure failure returns pause_immediately pipeline behavior', () => {
    ...
    const result = engine.raise(makeFailureContext({ cicdFailure: true }), makeRequestContext());
