...
const INJECTION_PATTERNS: { rule: string; re: RegExp }[] = [
  {
    ...
    re: /\bignore\s+(all\s+)?(the\s+)?(previous|prior|above|earlier|preceding)\s+(instructions?|context|prompts?|rules?)\b/i,
  },
  {
    rule: 'disregard_override',
    re: /\b(disregard|forget|discard|override|replace)\s+(all\s+)?(the\s+)?(previous|prior|above|earlier|preceding|your)\b/i,
  },
  ...
  {
    rule: 'from_now_on',
    re: /\bfrom\s+now\s+on\b[,:]?\s+(you|ignore|disregard|always|never|do\s+not|respond|reply|act|treat|forget|output|print)\b/i,
  },
  {
    rule: 'identity_override',
    re: /\b(you\s+are\s+now|you\s+must\s+now|act\s+as|pretend\s+to\s+be|roleplay\s+as)\b/i,
  },
  {
    rule: 'role_marker',
    re: /<\/?(system|assistant|user)>|<\|im_(start|end)\|>|(?:^|\n)\s*(System|Assistant|Human)\s*:/i,
  },
  {
...
function hasAssignedSecret(text: string): boolean {
  const re =
    /\b(api[_-]?key|secret|password|passwd|access[_-]?token|client[_-]?secret|bearer)\b\s*[:=]\s*['"]?([A-Za-z0-9/+_.\-]{12,})/gi;
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (/^(your|my|the|some|example|changeme|placeholder|redacted|xxx+|todo|none|null)/i.test(val))
      continue;
    return true;
  }
  return false;
...
export function enforceArtifactConstraints(
  ...
  if (!a.description || !a.description.trim())
    v.push({ rule: 'schema', field: 'description', detail: 'description is empty' });
  if (!a.body || !a.body.trim()) v.push({ rule: 'schema', field: 'body', detail: 'body is empty' });
  if (a.kind !== 'skill')
    v.push({
      rule: 'schema',
      field: 'kind',
      detail: `kind "${a.kind}" not implemented in Phase 2 (skills only)`,
    });
  ...
  if (firstBodyLine && firstBodyLine.trim() === '---') {
    v.push({
      rule: 'schema',
      field: 'body',
      detail: 'body must not start with a "---" delimiter line',
    });
  }
  ...
  if (a.name && (!NAME_RE.test(a.name) || a.name.includes('..'))) {
    v.push({
      rule: 'name_safety',
      field: 'name',
      detail: `name "${a.name}" must be kebab [a-z0-9-] with no ".."`,
    });
  } else if (a.name && WINDOWS_RESERVED_NAMES.has(a.name)) {
    v.push({
      rule: 'name_safety',
      field: 'name',
      detail: `name "${a.name}" is a reserved device name (invalid as a file stem on Windows)`,
    });
  }
  ...
  if (!isArtifactScope(a.scope)) {
    ...
    if (!isSafeScopeId(id)) {
      v.push({
        rule: 'scope_unsafe',
        field: 'scope',
        detail: `unsafe scope id "${id}" (traversal/charset)`,
      });
      ...
      if (!exists)
        v.push({
          rule: 'scope_exists',
          field: 'scope',
          detail: `${kind} "${id}" not found in ownership`,
        });
    }
  }
  ...
  for (const t of a.allowedTools.map(toolBase)) {
    if (t && !allowed.has(t)) {
      v.push({
        rule: 'tool_allowlist',
        field: 'allowed-tools',
        detail: `tool "${t}" not in the read-only allowlist (operator override required)`,
      });
    }
  }
  ...
  for (const { rule, re } of SECRET_PATTERNS) {
    if (re.test(scanText))
      v.push({
        ...
        detail: `possible secret (${rule})`,
      });
  }
  if (hasAssignedSecret(scanText)) {
    v.push({
      rule: 'secret:assigned_secret',
      field: 'description+body',
      detail: 'possible assigned secret value',
    });
  }
  for (const { rule, re } of INJECTION_PATTERNS) {
    if (re.test(scanText))
      v.push({
        rule: `injection:${rule}`,
        field: 'description+body',
        detail: `prompt-injection pattern (${rule})`,
      });
  }
