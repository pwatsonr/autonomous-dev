...
describe('TrustChangeManager', () => {
  ...
  test('concurrent changes use last-write-wins and emit superseded event', () => {
    ...
    const superseded = audit.events.filter((e) => e.event_type === 'trust_level_change_superseded');
  ...
  test('same-level request is a no-op', () => {
    const result = manager.requestChange('req-1', makeChangeRequest({ fromLevel: 2, toLevel: 2 }));
  ...
  test('emits trust_level_change_requested on requestChange', () => {
    ...
    const events = audit.events.filter((e) => e.event_type === 'trust_level_change_requested');
  ...
  test('emits trust_level_changed on resolveAtGateBoundary when change applied', () => {
    manager.requestChange('req-1', makeChangeRequest({ fromLevel: 2, toLevel: 0 }));
    manager.resolveAtGateBoundary('req-1', 2);

    const events = audit.events.filter((e) => e.event_type === 'trust_level_changed');
    expect(events).toHaveLength(1);
  ...
  test('confirmUpgrade throws when no pending change exists', () => {
    expect(() => manager.confirmUpgrade('nonexistent')).toThrow('No pending change found');
  });

  test('rejectUpgrade throws when no pending change exists', () => {
    expect(() => manager.rejectUpgrade('nonexistent')).toThrow('No pending change found');
  });
});
