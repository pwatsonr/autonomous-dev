...
describe('StatePersistence', () => {
  ...
  it('writes atomically via temp file, fsync, and rename', () => {
    ...
    const writes = writeLog.filter((e) => e.op === 'writeFile' || e.op === 'rename');
  ...
  it('does not leave partial file on crash during write', () => {
    ...
    const tmpFiles = Array.from(store.keys()).filter((k) => k.endsWith('.tmp'));
  ...
  it('creates state files in the correct directory structure', () => {
    ...
    persistence.writePipelineState('req-abc', makePipelineState('req-abc'));

    // Directory created: {baseDir}/{requestId}
    expect(dirs.has(`${BASE_DIR}/req-abc`)).toBe(true);

    // mkdirSync was called with recursive: true
    expect(mockFs.mkdirSync).toHaveBeenCalledWith(`${BASE_DIR}/req-abc`, { recursive: true });
  });
  ...
  it('does not throw on write failure (logs error)', () => {
    ...
    // Should not throw
    expect(() => persistence.writePipelineState('req-1', makePipelineState('req-1'))).not.toThrow();
