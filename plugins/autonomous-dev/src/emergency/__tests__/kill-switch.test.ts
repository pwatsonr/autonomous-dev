...
function createMockSnapshotCapture(): StateSnapshotCapture & {
  ...
  const captureAllSpy = jest.fn((ids: string[]) => ids.map((id) => makeSnapshot(id)));
...
describe('KillSwitch', () => {
  ...
  it('returns same result on second kill (idempotent)', async () => {
    ...
    const duplicateEvent = auditTrail.events.find((e) => e.event_type === 'kill_issued_duplicate');
  ...
  it('emits cancel_issued audit event on cancel', async () => {
    ...
    const cancelEvent = auditTrail.events.find((e) => e.event_type === 'cancel_issued');
  ...
  it('throws when reenable is called while system is running', () => {
    ...
    expect(killSwitch.getState()).toBe('running');
    expect(() => killSwitch.reenable('admin')).toThrow('Cannot re-enable: system is not halted');
  });
  ...
  it('emits system_reenabled audit event on reenable', async () => {
    ...
    const reenabledEvent = auditTrail.events.find((e) => e.event_type === 'system_reenabled');
  ...
  it('emits kill_issued audit event with snapshot path', async () => {
    ...
    const killEvent = auditTrail.events.find((e) => e.event_type === 'kill_issued');
