...
describe("JSON API routes", () => {
  ...
  test("GET /portal/homelab/api/entity with unknown id → 404", async () => {
    ...
    const res = await app.request(
      "/portal/homelab/api/entity?id=does-not-exist",
    );
...
describe("gated action buttons (#49)", () => {
  ...
  test("POST /portal/homelab/action/restart without token → 202 requiresConfirmation", async () => {
    ...
    const res = await app.request("/portal/homelab/action/restart", {
      method: "POST",
  ...
  test("POST /portal/homelab/action/redeploy without token → 202 with CONFIRM PERMANENT", async () => {
    const app = withAuth(appWithHomelab(), "alice");
    const res = await app.request("/portal/homelab/action/redeploy", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ entity_id: "service:platform:my-svc" }),
    });
  ...
  test("POST /portal/homelab/action/apply-autofix without token → 202 requiresConfirmation (#681)", async () => {
    const app = withAuth(appWithHomelab(), "alice");
    ...
    expect(body.requiresConfirmation).toBe(true);
    // #681: destructive → phrase is CONFIRM DESTROY (upgraded from CONFIRM PERMANENT).
  ...
  test("scale action with valid token executes handler and returns state", async () => {
    ...
    expect(
      body["action"] === "scale" || typeof body["error"] === "string",
    ).toBe(true);
  });
  ...
  test("audit entry emitted after action confirmation", async () => {
    ...
    const r1 = await wrapped.request("/portal/homelab/action/restart", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ entity_id: "service:platform:my-svc" }),
    });
    ...
    // Use token.
    const r2 = await wrapped.request("/portal/homelab/action/restart", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-contribution-action-token": token,
...
describe("homelabContribution shape", () => {
  ...
  test("restart action is reversible with minRole operator", () => {
    const restart = homelabContribution.actions?.find(
      (a) => a.id === "restart",
    );
  ...
  test("redeploy action is irreversible with minRole deployer", () => {
    const redeploy = homelabContribution.actions?.find(
      (a) => a.id === "redeploy",
    );
  ...
  test("apply-autofix action is destructive with minRole operator (#681)", () => {
    const applyFix = homelabContribution.actions?.find(
      (a) => a.id === "apply-autofix",
    );
    // #681: upgraded from "irreversible" to "destructive" — real apply mutation.
    expect(applyFix?.destructiveness).toBe("destructive");
