...
- **FR-A2** — Re-implements ONLY the tiny read shapes it needs (it cannot import the daemon plugin's `src`); the shapes are pinned + tolerant (unknown fields ignored; wrong types → safe default). A schema-drift in the daemon degrades a field, never crashes the portal.
- **FR-A3** — A cheap **ingestion-status** aggregate: `{ reposTotal, reposWithMemory, reposBlocked, questionsPending, proposalsPending, lastGraphSync? }` derived from ownership + memory dir listing + questions + proposals (no Neo4j call on the hot path).

### FR-B — Project/repo/team browser (view `onboard`)
- **FR-B1** — `GET /onboard` lists projects (with repo counts + tags) and repos, with **filters**: `?project=<id>`, `?tag=<k>=<v>` (the flexible team/domain dimension), `?q=<substring>` on repo id. Server-side filtered (scales to 100s) + paginated.
...
- **FR-C2** — **Live**: the page refreshes the status fragment via HTMX polling every ~5s (MVP — the ingest CLI does not emit SSE today; a true SSE feed is a fast-follow when ingestion broadcasts). The poll re-reads the cached readers (cheap). The "live" dot animates only when status changed.
...
- **FR-E1** — `POST /onboard/repo/:repo/enroll` and `.../unenroll` (CSRF-enforced) → `onboard-writers.ts` reads the manifest, flips `participate_in_auto_improvement` for that repo, and **atomically writes back preserving all other keys** (mirrors the daemon `writeOwnership` refuse-to-clobber-a-corrupt-manifest guard) → audits → returns the updated toggle fragment.
- **FR-E2** — The write is a read-modify-write on `~/.claude/autonomous-dev.json`; concurrency with the CLI is rare + last-writer-wins (documented, same stance as the daemon store). The toggle never enrolls in bulk; it is one repo per click.

### FR-F — IA + assets + security
- **FR-F1** — A new **"Onboard"** rail group in `ShellLayout` (the 3 views: Browser, Ingestion, Questions), with a pending-questions badge from the FR-A3 aggregate. New `ViewName`s + template dispatch + `activePathFor`.
...
- **NFR-2 (Test isolation):** all tests run under the `isolate-state-dir` preload; **never** read/write real `~/.autonomous-dev` or `~/.claude/autonomous-dev.json` (the 2026-06-12 webhook-wipe incident is the cautionary tale).
...
6. **P3.6** — rail nav "Onboard" group + badge; ViewName/dispatch/activePath; route tests; visual-regression goldens per view; css-coverage.
